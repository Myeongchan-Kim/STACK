...
void ModelClass::LoadFromPreLoadedData(PreLoadedData* data)
{
	m_indices = data->indices;
	m_vertices = data->vertices;
}

void ModelClass::LoadFromFile(std::string filename)
{
	std::ifstream fin;
...
void ModelClass::SetToBackground(float width, float height, XMFLOAT3 normal)
{
	m_vertices.clear();
	m_indices.clear();
